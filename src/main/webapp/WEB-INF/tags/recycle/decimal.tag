...
<form:input path="${path}" id="${empty id ? path : id}" name="${empty name ? path : name}" 
	htmlEscape="false" maxlength="${empty maxlength ? 30 : maxlength}" class="input-small number ${cssClass}" 
	style="${cssStyle}" value="${value}" disabled="${empty disabled ? false : disabled}"
	readonly="${empty readonly ? false : readonly}" onblur="formatDecimal(this, ${empty precision ? 2 : precision});${onblur}"/>
