...
<button id="${id}" class="btn ${empty cssClass ? 'btn-primary' : cssClass} ${not empty visible and visible eq 'false' ? 'hide' : ''}" style="${empty cssStyle ? '' : cssStyle}" ${empty disabled or disabled eq 'false' ? '' : 'disabled=\"disabled\"'} onclick="${not empty onclick ? onclick : ''}"><i class="Hui-iconfont">${iconValue}</i><tags:resourceMsg key="${key}" targetID="${targetID}"/></button>
