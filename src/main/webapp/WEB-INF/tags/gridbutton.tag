...
<span class="gridBtnSpan ${not empty visible and visible eq 'false' ? 'hide' : ''}"><button id="${id}" class="btn ${empty cssClass ? 'btn-primary' : cssClass}" style="${empty cssStyle ? '' : cssStyle}" ${empty disabled or disabled eq 'false' ? '' : 'disabled=\"disabled\"'} onclick="${not empty onclick ? onclick : ''}"><tags:msg key="${key}"/></button></span>
