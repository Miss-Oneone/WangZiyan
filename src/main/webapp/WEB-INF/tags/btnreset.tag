...
<button type="reset" class="btn btn-secondary ${not empty visible and visible eq 'false' ? 'hide' : ''}" id="${empty pageScope.id ? 'btnReset' : pageScope.id}"><i class="Hui-iconfont">&#xe68f;</i> <tags:msg key="${empty key ? 'btn_reset' : key}"/></button>
