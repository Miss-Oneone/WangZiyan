...
function addFavorite(name,site){
	try{window.external.addFavorite(site,name);}
		...
		try{window.sidebar.addPanel(name,site,"");}
			catch(e){alert("Failed to add favorite, please pressing Ctrl+D.");}
	}
}
/*收藏页面
<a title="收藏本页" href="javascript:addFavoritepage(0);">收藏本页</a>
*/
function addFavoritepage(){var sURL=window.location.href;var sTitle=document.title;try{window.external.addFavorite(sURL,sTitle);}catch(e){try{window.sidebar.addPanel(sTitle,sURL,"");}catch(e){alert("Failed to add favorite, please pressing Ctrl+D.");}}}
...
function setHome(obj){
  try{obj.style.behavior="url(#default#homepage)";obj.setHomePage(webSite);}
	  ...
	  catch(e){alert("Operation rejected by browser!");}
...
/*返回顶部*/
var $backToTopEle=$('<a href="javascript:void(0)" class="Hui-iconfont toTop" title="Back to top" alt="Back to top" style="display:none">&#xf0023;</a>').appendTo($("body")).click(function(){
	...
	$("a.preview").hover(
		function(){
			...
			if(midimg ==''){return false;}
				...
				var imgTitle = this.myTitle ? "<br />" + this.myTitle + " Preview image" : "";
				/*图片预加载*/
