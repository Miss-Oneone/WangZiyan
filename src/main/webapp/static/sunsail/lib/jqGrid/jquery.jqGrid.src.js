...
$.extend($.jgrid,{
	...
	from : function(source){
		...
		this.contains=function(f,v){
			 ...
			 */
			if(_useProperties){
				self._append(self._getStr('jQuery.jgrid.getAccessor(this,\''+f+'\')')+'.toUpperCase().indexOf("'+self._toStr(v).toUpperCase()+'",0) > -1');
			}else{
				self._append(self._getStr('this')+'.toUpperCase().indexOf("'+self._toStr(f).toUpperCase()+'",0) > -1');
			}
			/********** modify by wzf (2016212) end **********/
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		firstr = "<tr class='jqgfirstrow' role='row' style='height:1px'>";
