...
(function($){
	...
	$.fn.jBar = function(options){
		...
		return this.each(function(){
			...
			$(op.toggleBut, bar).select(function(){				
				DWZ.ui.sbar = false;
				$(op.splitBar).hide();
				...
				$(op.collapse,sbar).mouseover(function(){
					var sbarwidth = sbar.cssv("left") + sbar.outerWidth();
					if(bar.is(":hidden")) {
						$(op.toggleBut, bar).hide();
						bar.show().animate({left: sbarwidth}, 400);
						$(op.container).mouseover(_hideBar);
					}/* else {
						bar.animate({left: barleft}, 500, function(){
							bar.hide();
						});
					...
					function _hideBar() {
						$(op.container).unbind("mouseover", _hideBar);
						if (!DWZ.ui.sbar) {
							bar.animate({left: barleft}, 400, function(){
								bar.hide();
							});
						}
					}
			...
			$(op.toggleBut, bar).click(function(){
				...
				/* 用cookie记录左侧菜单默认显示/隐藏  */
				if ($.isFunction($.cookie)) $.cookie("barState", "hide");
				return false;
			});
			/* 根据cookie显示/隐藏左侧菜单  */
			if ($.isFunction($.cookie)){
				if ($.cookie("barState") == "hide") $(op.toggleBut, bar).trigger("select");
			}
			$(op.toggleBut, sbar).click(function(){
				DWZ.ui.sbar = true;
				sbar.animate({left: -25}, 400, function(){
					bar.show();
				});
				bar.animate({left: 5}, 800, function(){					
					...
					$(op.collapse, sbar).unbind('mouseover');
					$(window).trigger(DWZ.eventType.resizeGrid);
				});
				/* 用cookie记录左侧菜单默认显示/隐藏 */
				if ($.isFunction($.cookie)) $.cookie("barState", "show");
				return false;
			});
