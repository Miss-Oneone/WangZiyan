$(document).ready(function(){
    $("#send-cars-unContainer").jqGrid({
        ...
        colModel:[
            {name:'systemCode',index:'systemCode', width:'10%',align:'center',formatter:"showlink",
                          formatoptions:{baseLinkUrl:"send-cars-unContainer.html",target:"_self"},editable:true},
		...
		afterEditCell:function(rowid,name,val,iRow,iCol){ 
			...
			if(name=="amt"){ 
				...
				$("#send-cars-unContainer").jqGrid("setRowData",rowid,{"total":total}); 
				}); 
			} 
		 },
        pager:$('#gridPager')
	...
	for(var i=0;i<=mydata.length;i++)
		jQuery("#send-cars-unContainer").jqGrid('addRowData',i+1,mydata[i]);
});