$(document).ready(function(){
    $("#sendManagement").jqGrid({
        ...
        colModel:[
            {name:'A',index:'A', width:'20%',align:'center',formatter:"showlink",
                          formatoptions:{baseLinkUrl:"send-container.html",target:"_self"},editable:true},
		...
		afterEditCell:function(rowid,name,val,iRow,iCol){ 
			var rowdata = $("#sendManagement").jqGrid("getRowData",rowid); 
			var id=rowid+"_"+name; 
			if(name=="amt"){ 
				$("#"+id).keyup(function(e){ 
				var amt = $(this).val(); 
				...
				$("#sendManagement").jqGrid("setRowData",rowid,{"total":total}); 
				}); 
			} 
		 },
        pager:$('#gridPager')
	...
	for(var i=0;i<=mydata.length;i++)
		jQuery("#sendManagement").jqGrid('addRowData',i+1,mydata[i]);
});